  ...
  "The cluster receptionist" must {
    ...
    "handle a new incarnation of the same node well" in {
      ...
      try {
        ...
        val testKit3 = ActorTestKit(system1.name,
        ...
        try {
          ...
          system1.log.debug("Starting system3 at same hostname port as system2: [{}]",
                            clusterNode3.selfMember.uniqueAddress)
    ...
    "handle a new incarnation of the same node that is no longer part of same cluster" in {
      val testKit1 = ActorTestKit("ClusterReceptionistSpec-test-7",
      ...
      try {
        ...
        try {
          ...
          // FIXME cleanup this test, shouldn't need this much sleeping
          ...
          println(s"# sleeping ===========") // FIXME
          Thread.sleep(15000)
          println(s"# sleeping done ===========") // FIXME
