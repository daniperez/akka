    ...
    Behaviors.setup { ctx =>
        ...
        if (isLeader) {
          ...
          if (removals.nonEmpty) {
            if (ctx.log.isDebugEnabled)
              ctx.log.debug("ClusterReceptionist [{}] - Node(s) removed [{}], updating registry removing entries: [{}]",
                            cluster.selfAddress,
                            addresses.mkString(","),
                            removals
                              .map {
      ...
      def onCommand(cmd: Command): Behavior[Command] = cmd match {
        case ReceptionistMessages.Register(key, serviceInstance, maybeReplyTo) =>
          if (serviceInstance.path.address.hasLocalScope) {
            ...
            next(registry.addRegisteredService(key, serviceInstance))
          } else {
      ...
      def onInternalCommand(cmd: InternalCommand): Behavior[Command] = cmd match {
        ...
        case RegisteredActorTerminated(key, serviceInstance) =>
          val entry = Entry(serviceInstance, setup.selfSystemUid)
          ctx.log.debug("ClusterReceptionist [{}] - Registered actor terminated: [{}] [{}]",
                        cluster.selfAddress,
                        key.asServiceKey.id,
                        entry)
          val ddataKey = registry.ddataKeyFor(key)
          ...
          next(
            newState = registry
              .addTombstone(serviceInstance, setup.newTombstoneDeadline())
              .removeRegisteredService(key, serviceInstance))
        ...
        case ChangeFromReplicator(ddataKey, value) =>
          ...
          if (changedKeys.nonEmpty) {
            if (ctx.log.isDebugEnabled) {
              ctx.log.debug("ClusterReceptionist [{}] - Change from replicator: [{}], changes: [{}], tombstones [{}]",
                            cluster.selfAddress,
            ...
            changedKeys.foreach { changedKey =>
              ...
              if (subscribers.nonEmpty) {
                val listing =
                  ReceptionistMessages.Listing(serviceKey,
                                               newRegistry.activeActorRefsFor(serviceKey, selfUniqueAddress))
              ...
              if (tombstonedButReAdded.nonEmpty) {
                if (ctx.log.isDebugEnabled)
                  ctx.log.debug("ClusterReceptionist [{}] - Saw ActorRefs that were tomstoned [{}], re-removing.",
                                cluster.selfAddress,
              ...
              // FIXME remove this...
              val removedRegisteredServices = newRegistry.findRemovedRegisteredServices(changedKey.asServiceKey)
              if (removedRegisteredServices.nonEmpty) {
                ctx.log.error(
                  "ClusterReceptionist [{}] - Saw ActorRefs that were removed but registered locally [{}], re-adding.",
                  cluster.selfAddress,
                  removedRegisteredServices.mkString(", "))

                //                replicator ! Replicator.Update(ddataKey, EmptyORMultiMap, settings.writeConsistency) { registry =>
                //                  removedRegisteredServices.foldLeft(ServiceRegistry(registry)) { (acc, ref) =>
                //                    acc.addBinding(serviceKey, Entry(ref, setup.selfSystemUid))
                //                  }.toORMultiMap
                //                }
              }
        ...
        case NodeRemoved(uniqueAddress) =>
          if (uniqueAddress == selfUniqueAddress) {
            ...
            // anyway too late
            if (isLeader) {
              ctx.log.debug("ClusterReceptionist [{}] - Leader node observed removed node [{}]",
                            cluster.selfAddress,
                            uniqueAddress)
        ...
        case RemoveTick =>
          ...
          if (isLeader) {
            ...
            if (notInCluster.isEmpty) Behavior.same
              ...
              if (ctx.log.isDebugEnabled)
                ctx.log.debug("ClusterReceptionist [{}] - Leader node cleanup tick, removed nodes: [{}]",
                              cluster.selfAddress,
