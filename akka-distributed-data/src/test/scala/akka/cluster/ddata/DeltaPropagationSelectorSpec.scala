  ...
  "DeltaPropagationSelector" must {
    ...
    "keep track of deltas per node" in {
      ...
      val expected2 = DeltaPropagation(selfUniqueAddress,
                                       ...
                                       Some(17))
      val expected3 =
        DeltaPropagation(selfUniqueAddress, false, Map("C" -> Delta(DataEnvelope(deltaC), 1L, 1L)), Some(17))
    ...
    "bump version for each update" in {
      ...
      val expected1 = DeltaPropagation(selfUniqueAddress,
                                       false,
                                       Map("A" -> Delta(DataEnvelope(delta1.merge(delta2)), 1L, 2L)),
                                       Some(17))
      selector.collectPropagations() should ===(Map(nodes(0) -> expected1))
      selector.update("A", delta3)
      selector.currentVersion("A") should ===(3L)
      val expected2 =
        DeltaPropagation(selfUniqueAddress, false, Map("A" -> Delta(DataEnvelope(delta3), 3L, 3L)), Some(17))
    ...
    "merge deltas" in {
      ...
      selector.update("A", delta1)
      val expected1 =
        DeltaPropagation(selfUniqueAddress, false, Map("A" -> Delta(DataEnvelope(delta1), 1L, 1L)), Some(17))
      selector.collectPropagations() should ===(Map(nodes(0) -> expected1))
      ...
      val expected4 = DeltaPropagation(selfUniqueAddress,
                                       false,
                                       Map("A" -> Delta(DataEnvelope(delta2.merge(delta3)), 2L, 3L)),
                                       Some(17))
    ...
    "discard too large deltas" in {
      ...
      val expected = DeltaPropagation(selfUniqueAddress,
                                      false,
                                      Map("A" -> Delta(DataEnvelope(NoDeltaPlaceholder), 1L, 1000L)),
                                      Some(17))
      selector.collectPropagations() should ===(Map(nodes(0) -> expected))
    }
